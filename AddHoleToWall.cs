...
namespace HoleWallPlugin
{
    [Transaction(TransactionMode.Manual)]
    public class AddHoleToWall:IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            tr.Start("Расстановка отверстий в стенах");
            ...
            foreach (Duct d in ducts)
            {
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    heidht.Set(d.Diameter);
                }
            }

            foreach (Pipe p in pipes)
            {
                Line oCurve = (p.Location as LocationCurve).Curve as Line;
                XYZ oPoint = oCurve.GetEndPoint(0);
                XYZ oDirection = oCurve.Direction;

                List<ReferenceWithContext> oIntersections = referenceIntersector.Find(oPoint, oDirection)
                    .Where(x => x.Proximity <= oCurve.Length)
                    .Distinct(new ReferenceWithContextElementEqualityComparer())
                    .ToList();

                foreach (ReferenceWithContext oRefer in oIntersections)
                {
                    double oProximity = oRefer.Proximity;
                    Reference oReference = oRefer.GetReference();
                    Wall oWall = arDoc.GetElement(oReference.ElementId) as Wall;
                    Level oLevel = arDoc.GetElement(oWall.LevelId) as Level;
                    XYZ oPointHole = oPoint + (oDirection * oProximity);

                    FamilyInstance oHole = arDoc.Create.NewFamilyInstance(oPointHole, familySymbol, oWall, oLevel, StructuralType.NonStructural);
                    Parameter oWidth = oHole.LookupParameter("Ширина");
                    Parameter oHeidht = oHole.LookupParameter("Высота");
                    oWidth.Set(p.Diameter);
                    oHeidht.Set(p.Diameter);
                }
            }
            tr.Commit();
